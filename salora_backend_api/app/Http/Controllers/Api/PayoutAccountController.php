...
class PayoutAccountController extends BaseApiController
{
    ...
    public function store(Request $request){
        $data=$request->validate(['payment_method_id'=>'required|exists:payment_methods,id','account_name'=>'required|string|max:160','account_number'=>'nullable|string|max:190','phone'=>['nullable','regex:/^\d{10}$/'],'city'=>'nullable|string|max:120','branch'=>'nullable|string|max:190','instructions'=>'nullable|string|max:1500','is_default'=>'sometimes|boolean','is_active'=>'sometimes|boolean','qr'=>'nullable|image|mimes:png,jpg,jpeg,webp|max:2048']);
    ...
    public function update(Request $request,PayoutAccount $payoutAccount){abort_unless((int)$payoutAccount->user_id===(int)$request->user()->id,403);$data=$request->validate(['account_name'=>'sometimes|required|string|max:160','account_number'=>'nullable|string|max:190','phone'=>['nullable','regex:/^\d{10}$/'],'city'=>'nullable|string|max:120','branch'=>'nullable|string|max:190','instructions'=>'nullable|string|max:1500','is_default'=>'sometimes|boolean','is_active'=>'sometimes|boolean','qr'=>'nullable|image|mimes:png,jpg,jpeg,webp|max:2048']);return DB::transaction(function()use($request,$payoutAccount,$data){if($request->boolean('is_default'))PayoutAccount::where('user_id',$request->user()->id)->whereKeyNot($payoutAccount->id)->update(['is_default'=>false]);if($request->hasFile('qr')){if($payoutAccount->qr_path)Storage::disk('public')->delete($payoutAccount->qr_path);$data['qr_path']=$request->file('qr')->store('payout-qr','public');}unset($data['qr']);$payoutAccount->update($data);return $this->ok($payoutAccount->fresh('method'),'تم تحديث الحساب.');});}
