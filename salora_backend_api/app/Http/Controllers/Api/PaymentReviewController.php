...
{
    public function index(Request $request){return $this->ok(PaymentProof::with(['invoice.booking.venue','invoice.customer:id,name,email,phone','method','payoutAccount','paymentAdjustment'])->whereHas('invoice',fn($q)=>$q->where('payee_id',$request->user()->id))->latest()->get());}
