...
Route::middleware(['auth:sanctum','account.active','role:customer'])->prefix('customer')->group(function(){
    Route::get('/invoices',[InvoicePaymentController::class,'index']);Route::get('/invoices/{invoice}',[InvoicePaymentController::class,'show']);Route::post('/invoices/{invoice}/payment-proof',[InvoicePaymentController::class,'uploadProof']);Route::post('/invoices/{invoice}/refund',[InvoicePaymentController::class,'requestRefund']);Route::post('/refunds/{refund}/confirm',[InvoicePaymentController::class,'confirmRefund']);Route::post('/refunds/{refund}/dispute',[InvoicePaymentController::class,'disputeRefund']);
});
Route::middleware(['auth:sanctum','account.active','role:owner,provider'])->group(function(){
    ...
    Route::get('/business/payments',[PaymentReviewController::class,'index']);Route::post('/business/payments/{payment}/approve',[PaymentReviewController::class,'approve']);Route::post('/business/payments/{payment}/reject',[PaymentReviewController::class,'reject']);Route::get('/business/refunds',[PaymentReviewController::class,'refunds']);Route::post('/business/refunds/{refund}/proof',[PaymentReviewController::class,'uploadRefundProof']);
});
