...
const bookingStatusFromApi = (status = "") => {
  ...
  return normalizeStatus(status || "pending_owner_review");
};
...
const venueFromApi = (v = {}) => {
  ...
  return {
    id: String(v.id ?? ""),
    name: v.name_ar || v.name_en || v.name || "صالة",
    city: v.city || "",
    address: v.address || "",
    mapUrl: v.map_url || "",
    latitude: v.latitude === null || v.latitude === undefined ? null : Number(v.latitude),
    longitude: v.longitude === null || v.longitude === undefined ? null : Number(v.longitude),
    ...
    description: v.description_ar || v.description_en || "",
    ...
    pendingRevision: v.pending_revision || null,
...
const bookingFromApi = (b = {}) => ({
  ...
  refundPercentage: Number(b.refund_percentage || 0),
  refundedSyp: Number(b.refunded_syp || 0),
  financialStatus: String(b.financial_status || "")
});
...
export function AppProvider({ children }) {
  ...
  const derivedApprovals = useMemo(() => {
    ...
    const offerRequests = offers.filter((o) => o.status === "Pending").map((o) => ({
      id: `offer-${o.id}`,
      type: "Offer",
      offerId: o.id,
      title: o.title,
      requester: "مالك صالة / مقدم خدمة",
      status: "Pending",
      createdAt: o.startsAt || today(),
      details: `عرض على ${o.target || "خدمة أو صالة"} • ${o.type === "percentage" ? `${o.discount}%` : formatPricePair(o.discount)} • من ${o.startsAt || "غير محدد"} إلى ${o.endsAt || "غير محدد"}`
    }));
    const paymentRequests = currentRole === ROLES.OWNER ? bookings.filter((b) => b.ownerId === currentUser.id && (b.paymentStatus === PAYMENT_STATUS.PENDING_ADMIN_VERIFICATION || b.paymentStatus === "Proof Uploaded")).map((b) => ({ id: `payment-${b.id}`, type: "Payment Proof", bookingId: b.id, paymentProofId: b.paymentProofId, title: `حجز ${b.id}`, requester: b.customer, status: "Pending", createdAt: b.date, details: `إثبات دفع مرفوع لصالة ${b.venue}. المبلغ: ${formatPricePair(b.invoiceTotal || b.amount)}` })) : [];
    return [...ownerRequests.filter((r) => r.status === "Pending"), ...venueRequests, ...serviceRequests, ...offerRequests, ...paymentRequests, ...approvals.filter((a) => a.status === "Pending")];
      ...
      if (videoFiles.length) {
        try {
          ...
          if (rawVideoUrls.length) {
            mapped = { ...mapped, videoUrls: rawVideoUrls.map(assetUrl), rawVideoUrls, videos: rawVideoUrls.length };
            setVenues((prev) => prev.map((venue) => venue.id === mapped.id ? mapped : venue));
          }
        } catch (uploadError) {
  ...
  const updateVenue = async (id, updatedData) => {
    try {
      ...
      let mapped = venueFromApi(publishedVenue);
      ...
      const removedImageIds = Array.isArray(updatedData.removedImageIds) ? updatedData.removedImageIds : [];
      let finalImageUrls = Array.isArray(updatedData.retainedRawImageUrls) ? [...updatedData.retainedRawImageUrls] : [];

      let uploadedUrls = [];
      if (imageFiles.length) {
        const formData = new FormData();
        imageFiles.forEach((file) => formData.append("images[]", file));
        if (updatedData.makeNewImagesCover) formData.append("is_main", "1");
        const uploaded = await dashboardApi.owner.uploadVenueImage(id, formData);
        uploadedUrls = asArray(uploaded?.uploaded_urls).filter(Boolean);
        finalImageUrls = asArray(uploaded?.final_image_urls).length
          ? asArray(uploaded.final_image_urls)
          : asArray(uploaded?.images).map((image) => image.image_url).filter(Boolean);
      }

      for (const imageId of removedImageIds) {
        const response = await dashboardApi.owner.deleteVenueImage(id, imageId);
        if (asArray(response?.final_image_urls).length) finalImageUrls = asArray(response.final_image_urls);
        else if (asArray(response?.images).length) finalImageUrls = asArray(response.images).map((image) => image.image_url).filter(Boolean);
      }

      const desiredImageOrder = Array.isArray(updatedData.imageOrder)
        ? updatedData.imageOrder.map((entry) => entry?.kind === "new"
          ? uploadedUrls[Number(entry.index)]
          : entry?.url).filter(Boolean)
        : [];

      if (desiredImageOrder.length && desiredImageOrder.length === finalImageUrls.length) {
        await dashboardApi.owner.reorderVenueImages(id, desiredImageOrder);
        finalImageUrls = desiredImageOrder;
      } else if (updatedData.coverImageId && !imageFiles.length) {
        await dashboardApi.owner.setMainVenueImage(id, updatedData.coverImageId);
      }

      if (imageFiles.length || removedImageIds.length || updatedData.coverImageId) {
        setRefreshKey((key) => key + 1);
      }
      const videoFiles = Array.isArray(updatedData.videoFiles) ? updatedData.videoFiles : [];
      const removedVideoIds = Array.isArray(updatedData.removedVideoIds) ? updatedData.removedVideoIds : [];
      let finalVideoUrls = Array.isArray(updatedData.retainedRawVideoUrls) ? [...updatedData.retainedRawVideoUrls] : [];
      ...
      if (videoFiles.length) {
        ...
        finalVideoUrls = asArray(uploaded?.final_video_urls).length
          ? asArray(uploaded.final_video_urls)
          : asArray(uploaded?.videos).map((video) => video.video_url || video.url).filter(Boolean);
      }
      for (const videoId of removedVideoIds) {
        const response = await dashboardApi.owner.deleteVenueVideo(id, videoId);
        if (asArray(response?.final_video_urls).length) finalVideoUrls = asArray(response.final_video_urls);
        else if (asArray(response?.videos).length) finalVideoUrls = asArray(response.videos).map((video) => video.video_url || video.url).filter(Boolean);
      }
      const desiredVideoOrder = Array.isArray(updatedData.videoOrder)
        ? updatedData.videoOrder.map((entry) => entry?.kind === "new" ? uploadedVideoUrls[Number(entry.index)] : entry?.url).filter(Boolean)
        : [];
      if (desiredVideoOrder.length === finalVideoUrls.length && (desiredVideoOrder.length || removedVideoIds.length)) {
        await dashboardApi.owner.reorderVenueVideos(id, desiredVideoOrder);
        finalVideoUrls = desiredVideoOrder;
      }
      if (videoFiles.length || removedVideoIds.length || desiredVideoOrder.length) setRefreshKey((key) => key + 1);
  ...
  const ownerApproveBooking = async (id) => {
    const target = bookings.find((booking) => booking.id === String(id));
    if (!target || target.status !== BOOKING_STATUS.PENDING_OWNER_REVIEW) {
      window.alert("يمكن قبول الحجوزات التي تنتظر موافقة المالك فقط.");
      return null;
    }
    try {
      const saved = await dashboardApi.owner.approveBooking(id);
      const mapped = applyBookingFromApi(saved, id);
      logAction("Owner approved booking", id, "booking", "Owner");
      return mapped;
    } catch (error) {
      return showMutationError(error, "تعذر قبول الحجز.");
    }
  };

  const ownerRejectBooking = async (id) => {
    const reason = window.prompt("اكتب سبب رفض الحجز:");
    if (!reason?.trim()) return null;
    try {
      const saved = await dashboardApi.owner.rejectBooking(id, reason.trim());
      const mapped = applyBookingFromApi(saved, id);
      logAction("Owner rejected booking", id, "booking", "Owner");
      return mapped;
    } catch (error) {
      return showMutationError(error, "تعذر رفض الحجز.");
    }
  };
  ...
  const updatePaymentStatus = async (id, paymentStatus, rejectionReason = "") => {
    if (currentRole !== ROLES.ADMIN) {
      window.alert("مراجعة إثباتات الدفع من صلاحيات مدير النظام في هذه النسخة.");
      return null;
    }
    const target = bookings.find((booking) => booking.id === String(id));
    ...
    try {
      const saved = paymentStatus === PAYMENT_STATUS.VERIFIED
        ? await dashboardApi.admin.approvePayment(paymentId)
        : await dashboardApi.admin.rejectPayment(
            paymentId,
      ...
      logAction(`Payment ${paymentStatus}`, id, "payment", "Admin");
      return saved;
    } catch (error) {
