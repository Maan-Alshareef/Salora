...
export default function SchedulingCalendar() {
  const { bookings, venues, arabicLabel } = useApp();
  const now = new Date();
  ...
  const visibleBookings = useMemo(() => bookings
    ...
    .sort((a, b) => `${a.date} ${a.time || ""}`.localeCompare(`${b.date} ${b.time || ""}`)), [bookings, year, month, selectedVenue]);
  ...
  return (
    <div className="space-y-6 pb-12 text-white" dir="rtl">
      <div className="rounded-3xl border border-blue-400/20 bg-blue-500/10 p-6">
        <h1 className="text-3xl font-black bg-clip-text text-transparent bg-gradient-to-r from-blue-300 to-white">📅 تقويم الحجوزات</h1>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
        <div className="rounded-3xl border border-white/10 bg-white/[.04] p-5"><div className="text-xs text-slate-500">كل الحجوزات</div><div className="mt-2 text-3xl font-black">{bookings.length}</div></div>
        <div className="rounded-3xl border border-white/10 bg-white/[.04] p-5"><div className="text-xs text-slate-500">حجوزات هذا الشهر</div><div className="mt-2 text-3xl font-black text-blue-300">{visibleBookings.length}</div></div>
        ...
        <div className="rounded-3xl border border-white/10 bg-white/[.04] p-5"><div className="text-xs text-slate-500">الصالة</div><div className="mt-2 text-lg font-black text-slate-100">{selectedVenue === "All" ? "كل الصالات" : venues.find((v) => String(v.id) === String(selectedVenue))?.name || "صالة محددة"}</div></div>
      </div>
